...
void YOLOv5TRTDetector::initConfig(std::string enginefile, float conf_threshold, float score_threshold) {
...
void YOLOv5TRTDetector::detect(cv::Mat& frame, std::vector<DetectResult>& results) {
	...
	cv::Mat tensor = cv::dnn::blobFromImage(image, 1.0f / 255.f, cv::Size(this->input_w, this->input_h), cv::Scalar(0, 0, 0), true, false);
	...
	//GPU显存到内存  一开始就是这里错误了，cudaMemcpyHostToDevice和cudaMemcpyDeviceToHost
	...
	for (int i = 0; i < det_output.rows; i++) {
		...
		cv::Mat classes_scores = det_output.row(i).colRange(4, output_h); //此处应该是84 类别数
		cv::Point classIdPoint;
		double score;
		minMaxLoc(classes_scores, 0, &score, 0, &classIdPoint);
		//std::cout << classes_scores << std::endl;  //得分等信息全为0，读取不到东西
	...
	for (size_t i = 0; i < indexes.size(); i++) {
		...
		dr.score = confidences[idx]; //这里原文是conf
		dr.box = boxes[idx];
